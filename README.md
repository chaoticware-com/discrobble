...
- Implementation status: the Worker runtime scaffold, Gradle/KMP root, shared module tree, and native auth shells now exist. The Worker and both native shells now implement the Last.fm and Discogs auth spikes end to end, and the Worker exposes signed Last.fm write proxy endpoints with normalized error handling. Discogs collection reads, barcode search, and the remaining provider spikes are still pending.
