...
- Implementation status: the Worker runtime scaffold, Gradle/KMP root, shared module tree, and native auth shells now exist. The Worker and both native shells now implement the Last.fm auth spike end to end, and the Worker now exposes signed `track.updateNowPlaying` and `track.scrobble` proxy endpoints with normalized error handling. Discogs auth and the remaining provider spikes are still pending.
